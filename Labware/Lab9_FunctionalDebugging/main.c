...
int main(void){  unsigned long i,last,now;
	...
	while(1){
		...
		if(  (SW1==0x00) || (SW2==0x00) ){
			...
			GPIO_PORTF_DATA_R = GPIO_PORTF_DATA_R & 0xFD;   // output 
		}
		
		if( (lastInputs!= (GPIO_PORTF_DATA_R & 0x13) ) && (i<50)){
			now = NVIC_ST_CURRENT_R;
			Time[i] = (last-now)&0x00FFFFFF;  // 24-bit time difference
			Data[i] = GPIO_PORTF_DATA_R&0x13; // record PF0, PF1, PF4
			last = now;
			
			lastInputs = GPIO_PORTF_DATA_R & 0x13;
			i++;
		}
